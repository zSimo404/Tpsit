...
bool check(int vet[],int cerc,int dim){       //funzione che controlla che nel random non ci siano numeri uguali 
    bool ok=false;     
...
void printRand(Canzone playlist[],int dim){    //procedura che crea vettore di appoggio e carica in numeri random sopra
    int vet[dim];     //dichoara vettore di interi di dimensione uguale al numero delle canzoni
    srand(time(NULL));   //
    for(int i=0;i<dim;i++){   //for ce cicla tutto il vettore
        do{
            vet[i]=rand()%(dim-1+1);    //randomizza numero 
        }while(check(vet,vet[i],i));    //chiamata della funzione check che serve per controllare se il numero è già stato usato
    }
